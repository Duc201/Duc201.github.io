...
function reload(init) {
  ...
  // T(i,i+3) = 1 // T(i,i+4) = 2 // T(i,i+8) = 3 
  // T(7,28) = 5 // T(12,46) = 6 // T(60,68) = 4
  // T[6][27] = 5; T[27][6] = 5;
  // T[11][45] = 6; T[45][11] = 6;
  // T[59][67] = 4; T[67][59] = 4;
  for (let i = 0; i < nodeNumber; i++) {
...
function lineNode(a, b, color = 255) {   // mau lien ket cac node
  stroke(color);
  strokeWeight(1.5);
  line(
    Math.round(a.x / 2),
...
function lineBlackboneNode(a, b, color = 255) {   // mau lien ket cac node
  stroke(color);
  strokeWeight(2);
  line(
    Math.round(a.x / 2),
...
function drawBackboneCircle(e, R_backbone, color = 255) {    // mau cua duong tron
  noFill();
  stroke(color);
...
function setup() {
  ...
  if (DRAW_MAX_DISTANCE) {
    lineNode(max_distance.node1, max_distance.node2, color(255, 0, 0)); // mau cua duong max
  }
  ...
  $("#nodes").scroll(function () {
    if ($(this).scrollTop() == 0) {
      ...
      $(this).css("border-top", "1px solid rgba(34,36,38,.1)"); // mau cua cac so trong bang
    }
  });
  ...
  if (!MENTOR_1) {
    ...
    P.forEach((e, i) => {
      if (e != i) {
        ...
        if (!HIDE_NO_TRAFFIC || T_b[e][i] > 0) {
          lineBlackboneNode(backbones[e], backbones[i], 
            color(255, 204, 0)); // mau dong backbone
  ...
  //ve node
  drawCenterNode(points[min_moment_index], color(255, 69, 0)); // mau node trung tâm
  points.forEach((p, i) => {
  ...
  if (MENTOR_1) {
    ...
    pair.forEach((e) => {
      ...
      if (e.hop >= 2 && T_b[e.s][e.d] > 0) {
        if (u >= u_min) {
          lineNode(backbones[e.s], backbones[e.d], 
            color(17, 0, 255));  // mau liên kết trực tiếp
            // color(111, 255, 0));
        ...
        lineNode(backbones[e.s], backbones[e.d], 
          color(17, 0, 255)); // liên kết trực tiếp
          // color(111, 255, 0));
        links[e.s][e.d] = links[e.d][e.s] = {
          n: n,
          u: u,
  ...
  points.forEach((e, i) => {
    if (e.is_access) {
      fill(color(129, 231, 71)); // mau cac nut thuong 
      textSize(13);
  ...
  backbones.forEach((e, i) => {
    fill(color(38, 215, 214)); // mau cac nut backbone
    textSize(15);
